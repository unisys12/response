...
func (uq *UserQuery) QueryOauthConnections() *OAuthConnectionQuery {
	query := &OAuthConnectionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryPeople() *PersonQuery {
	query := &PersonQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOauthConnections: uq.withOauthConnections.Clone(),
		withPeople:           uq.withPeople.Clone(),
...
func (uq *UserQuery) WithOauthConnections(opts ...func(*OAuthConnectionQuery)) *UserQuery {
	query := &OAuthConnectionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withOauthConnections = query
	return uq
}
...
func (uq *UserQuery) WithPeople(opts ...func(*PersonQuery)) *UserQuery {
	query := &PersonQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			uq.withMetadata != nil,
			uq.withOauthConnections != nil,
			uq.withPeople != nil,
		}
	)
	if uq.withMetadata != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withOauthConnections; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.OauthConnections = []*OAuthConnection{}
		}
		query.withFKs = true
		query.Where(predicate.OAuthConnection(func(s *sql.Selector) {
			s.Where(sql.InValues(user.OauthConnectionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_oauth_connections" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_oauth_connections" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OauthConnections = append(node.Edges.OauthConnections, n)
		}
	}

	if query := uq.withPeople; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Person(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PeopleColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_people
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_people" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_people" returned %v for node %v`, *fk, n.ID)
			}
