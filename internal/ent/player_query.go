...
type PlayerQuery struct {
	...
	withServers     *GameServerQuery
	withIdentifiers *PlayerIdentifierQuery
	withFKs         bool
...
func (pq *PlayerQuery) QueryServers() *GameServerQuery {
	query := &GameServerQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlayerQuery) QueryIdentifiers() *PlayerIdentifierQuery {
	query := &PlayerIdentifierQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlayerQuery) Clone() *PlayerQuery {
	...
	return &PlayerQuery{
		...
		withServers:     pq.withServers.Clone(),
		withIdentifiers: pq.withIdentifiers.Clone(),
...
func (pq *PlayerQuery) WithServers(opts ...func(*GameServerQuery)) *PlayerQuery {
	query := &GameServerQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withServers = query
	return pq
}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context) ([]*Player, error) {
	...
	if query := pq.withMetadata; query != nil {
		...
		for i := range nodes {
			if nodes[i].player_metadata == nil {
				continue
			}
			fk := *nodes[i].player_metadata
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_metadata" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withServers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Player, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Servers = []*GameServer{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(player.ServersPrimaryKey[0], fks...))
			},
		...
		query.Where(gameserver.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
	...
	if query := pq.withIdentifiers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Player)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Identifiers = []*PlayerIdentifier{}
		}
		query.withFKs = true
		query.Where(predicate.PlayerIdentifier(func(s *sql.Selector) {
			s.Where(sql.InValues(player.IdentifiersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.player_identifiers
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "player_identifiers" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_identifiers" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Identifiers = append(node.Edges.Identifiers, n)
		}
	}
...
func (pq *PlayerQuery) ForUpdate(opts ...sql.LockOption) *PlayerQuery {
...
func (pq *PlayerQuery) ForShare(opts ...sql.LockOption) *PlayerQuery {
	if pq.driver.Dialect() == dialect.Postgres {
		pq.Unique(false)
	}
	pq.modifiers = append(pq.modifiers, func(s *sql.Selector) {
