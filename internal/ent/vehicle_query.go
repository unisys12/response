...
type VehicleQuery struct {
	...
	withRegistrations *VehicleRegistrationQuery
	withMake          *VehicleMakeQuery
	withModel         *VehicleModelQuery
	withMajorColor    *VehicleColorQuery
	withMinorColor    *VehicleColorQuery
	withClass         *VehicleClassQuery
	withFKs           bool
...
func (vq *VehicleQuery) QueryMetadata() *MetadataQuery {
	query := &MetadataQuery{config: vq.config}
...
func (vq *VehicleQuery) QueryRegistrations() *VehicleRegistrationQuery {
	query := &VehicleRegistrationQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryMake chains the current query on the "make" edge.
func (vq *VehicleQuery) QueryMake() *VehicleMakeQuery {
	query := &VehicleMakeQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VehicleQuery) QueryModel() *VehicleModelQuery {
	query := &VehicleModelQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryMajorColor chains the current query on the "major_color" edge.
func (vq *VehicleQuery) QueryMajorColor() *VehicleColorQuery {
	query := &VehicleColorQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryMinorColor chains the current query on the "minor_color" edge.
func (vq *VehicleQuery) QueryMinorColor() *VehicleColorQuery {
	query := &VehicleColorQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (vq *VehicleQuery) QueryClass() *VehicleClassQuery {
	query := &VehicleClassQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VehicleQuery) WithMetadata(opts ...func(*MetadataQuery)) *VehicleQuery {
	query := &MetadataQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withMetadata = query
	return vq
}
...
func (vq *VehicleQuery) WithRegistrations(opts ...func(*VehicleRegistrationQuery)) *VehicleQuery {
	query := &VehicleRegistrationQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withRegistrations = query
	return vq
}

// WithMake tells the query-builder to eager-load the nodes that are connected to
// the "make" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VehicleQuery) WithMake(opts ...func(*VehicleMakeQuery)) *VehicleQuery {
	query := &VehicleMakeQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VehicleQuery) WithModel(opts ...func(*VehicleModelQuery)) *VehicleQuery {
	query := &VehicleModelQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithMajorColor tells the query-builder to eager-load the nodes that are connected to
// the "major_color" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VehicleQuery) WithMajorColor(opts ...func(*VehicleColorQuery)) *VehicleQuery {
	query := &VehicleColorQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withMajorColor = query
	return vq
}

// WithMinorColor tells the query-builder to eager-load the nodes that are connected to
// the "minor_color" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VehicleQuery) WithMinorColor(opts ...func(*VehicleColorQuery)) *VehicleQuery {
	query := &VehicleColorQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withMinorColor = query
	return vq
}
...
func (vq *VehicleQuery) WithClass(opts ...func(*VehicleClassQuery)) *VehicleQuery {
	query := &VehicleClassQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VehicleQuery) sqlAll(ctx context.Context) ([]*Vehicle, error) {
	var (
		...
		loadedTypes = [7]bool{
			vq.withMetadata != nil,
			vq.withRegistrations != nil,
			vq.withMake != nil,
			vq.withModel != nil,
			vq.withMajorColor != nil,
			vq.withMinorColor != nil,
			vq.withClass != nil,
		}
	)
	if vq.withMetadata != nil || vq.withMake != nil || vq.withModel != nil || vq.withMajorColor != nil || vq.withMinorColor != nil || vq.withClass != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withMetadata; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_metadata == nil {
				continue
			}
			fk := *nodes[i].vehicle_metadata
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_metadata" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := vq.withRegistrations; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Vehicle)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Registrations = []*VehicleRegistration{}
		}
		query.withFKs = true
		query.Where(predicate.VehicleRegistration(func(s *sql.Selector) {
			s.Where(sql.InValues(vehicle.RegistrationsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.vehicle_registrations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "vehicle_registrations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_registrations" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Registrations = append(node.Edges.Registrations, n)
		}
	}

	if query := vq.withMake; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_make_vehicles == nil {
				continue
			}
			fk := *nodes[i].vehicle_make_vehicles
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(vehiclemake.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_make_vehicles" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Make = n
			}
		}
	}

	if query := vq.withModel; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_model_vehicles == nil {
				continue
			}
			fk := *nodes[i].vehicle_model_vehicles
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(vehiclemodel.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_model_vehicles" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := vq.withMajorColor; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_color_vehicles_major == nil {
				continue
			}
			fk := *nodes[i].vehicle_color_vehicles_major
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(vehiclecolor.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_color_vehicles_major" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.MajorColor = n
			}
		}
	}

	if query := vq.withMinorColor; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_color_vehicles_minor == nil {
				continue
			}
			fk := *nodes[i].vehicle_color_vehicles_minor
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_color_vehicles_minor" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.MinorColor = n
			}
		}
	}

	if query := vq.withClass; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Vehicle)
		for i := range nodes {
			if nodes[i].vehicle_class_vehicles == nil {
				continue
			}
			fk := *nodes[i].vehicle_class_vehicles
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(vehicleclass.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vehicle_class_vehicles" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (vq *VehicleQuery) ForUpdate(opts ...sql.LockOption) *VehicleQuery {
