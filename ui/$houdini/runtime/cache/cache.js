...
export class Cache {
	...
	list(name, id) {
		...
		const handler =
			(_a = this._lists.get(name)) === null || _a === void 0 ? void 0 : _a.get(id || rootID);
		if (!handler) {
			throw new Error(
	...
	getData(parent, selection, variables) {
		...
		for (const [attributeName, { type, keyRaw, fields }] of Object.entries(selection)) {
			...
			// we are looking at a scalar or some other type we don't recognize
				...
				if (
					(_b = (_a = this._config.scalars) === null || _a === void 0 ? void 0 : _a[type]) ===
						null || _b === void 0
						? void 0
						: _b.unmarshal
				) {
		...
		for (const { type, keyRaw, fields, list, filters } of Object.values(selection)) {
			...
			if (!isScalar(this._config, type)) {
				...
				if (list && fields) {
					...
					(_a = this._lists.get(list.name)) === null || _a === void 0
						? void 0
						: _a.set(
	...
	_write({ rootID, selection, recordID, data, variables, specs, applyUpdates }) {
		var _a;
		...
		for (const [field, value] of Object.entries(data)) {
			...
			if (value instanceof Object && !Array.isArray(value) && fields) {
				...
				const embedded =
					((_a = this.idFields(linkedType)) === null || _a === void 0
						? void 0
						: _a.filter((field) => typeof value[field] === 'undefined').length) > 0;
			...
			// the value could be a list
				...
				if (applyUpdates && update) {
					...
					if (key === 'edges') {
						...
						oldIDs = oldIDs.filter((id) => !(newIDs.includes(id) && emptyEdges.includes(id)));
					}
			...
			// the value is neither an object or a list so its a scalar
				...
				if (JSON.stringify(value) !== JSON.stringify(record.getField(key))) {
					...
					if (Array.isArray(value) && applyUpdates && update) {
						...
						if (update === 'append') {
							newValue = (record.getField(key) || []).concat(value);
						}
			...
			for (const operation of operations || []) {
				...
				// only insert an object into a list if we're adding an object with fields
					...
					this.list(operation.list, parentID).when(operation.when).remove(value, variables);
				}
	...
	extractNestedListIDs({
		...
		for (const [i, entry] of value.entries()) {
			...
			const embedded =
				((_a = this.idFields(linkedType)) === null || _a === void 0
					? void 0
					: _a.filter((field) => typeof entry[field] === 'undefined').length) > 0;
			...
			if (key === 'edges' && entryObj.node && entryObj.node.__typename) {
		...
		for (const spec of specs) {
			...
			spec.set(
				this.getData(
					rootRecord,
					spec.selection,
					(_a = spec.variables) === null || _a === void 0 ? void 0 : _a.call(spec)
				)
			);
		}
	}
